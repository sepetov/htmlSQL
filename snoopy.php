...
class Snoopy
{
	...
	/**** Private variables ****/
	// Попробовать эти переменные "в лоб" перевести в private, ничего не должно сломаться.
	...
	function fetch($URI)
	{
		...
		switch(strtolower($URI_PARTS["scheme"]))
		{
			...
			case "https":
				if(!$this->curl_path)
				{
					$this->error = 'Не указан путь к исполняемому файлу curl' . PHP_EOL;
					return false;
				}

				if(function_exists("is_executable"))
				{
				    if (is_executable($this->curl_path) == false)
					{
						$this->error = 'Не найден исполняемый файл библиотеки curl.' . PHP_EOL;
				        return false;
					}
				}
				$this->host = $URI_PARTS["host"];
